...
int main(int argc, char **argv)
{
    ...
    cout<<"energy "<<arma::sum(eval1(arma::find(eval1<0)))<<endl;

    auto green=[&](int i, int j, double t, bool is_greater) {
        cx_vec phase=arma::exp(eval1*cx_double(0,-t));
        rowvec jket=evec1.row(j);
        rowvec iket=evec1.row(i);
        cx_double sum=0;
        for(auto a=0; a<eval1.size(); a++)
           if ((is_greater && eval1[a]>=0) ||
                (!is_greater && eval1[a]<0)) sum += std::conj(iket[a])*jket[a]*phase(a);
        return sum;
    };
    ...
    for(auto i=0; i*dt<=len; i++) {
        ...
        out<<t<<" 1 1 "<< g.real() <<" "<< g.imag()<< " 0 0"<<endl;
    }
